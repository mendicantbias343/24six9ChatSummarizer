<template>
    <div class="flex flex-col justify-center items-start px-3 w-full">
      <div
        class="date-navigation text-md text-primary-focus flex-row flex justify-around w-full items-stretch text-2xl sticky top-0 bg-base-100 z-20 py-3"
      >
        <div  v-bind:class="previous_date?'next cursor-pointer':'next cursor-pointer grayscale'"><NuxtLink v-bind:to="previous_date">⏮️</NuxtLink></div>
        <div class="current-date w-4/5 text-center text-lg">
          {{ formattedDate}}
        </div>
        <div   v-bind:class="next_date ? 'next cursor-pointer' : 'next cursor-pointer grayscale'"><NuxtLink v-bind:to="next_date">⏭️</NuxtLink></div>
      </div>
      <div class="topics-holder w-full mt-10">
        ...
        <div class="topic flex flex-col lg:grid lg:grid-cols-3 lg:gap-4">
          <!-- card start-->
          <div
            class="card card-compact bg-base-200 bg-opacity-60 mt-5"
  ...
  <li v-for="link in data.links" class="mt-3 text-primary underline underline-offset-2 list-disc">
        <a v-bind:href="link" target="_blank" >{{ link.slice(0,35) }}...</a></li>
      </ul>
  
        
      </div>
  
      <div class="hiring-requests flex flex-col mt-10" v-if="data.hiring_requests.length != 0">
        <h1 class="text-xl text-">Hiring Requests</h1>
        <ol class="pl-5 list-decimal">
        
        <li v-for="req in data.hiring_requests" class="w-full overflow-hidden mt-3 h-[2ch] text-accent-content list-decimal ">
              {{ req }}</li>
            </ol>
      </div>
    </div>
...
function formatDate(inputDate) {
  ...
  const fullWeekdayName = dateObj.toLocaleDateString('en-US', { weekday: 'long' });
